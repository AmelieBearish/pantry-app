...
export default function App() {
  ...
  if (authLoading) {
    return (
      <div style={{ minHeight: "100vh", background: "#f7f5f0", display: "flex", alignItems: "center", justifyContent: "center", color: "#999" }}>
        読み込み中...
      </div>
    );
  }
  ...
  if (!user) {
    return (
      <div style={{ minHeight: "100vh", background: "#f7f5f0", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: 16 }}>
        <div style={{ color: "#2d2926", fontSize: 22, fontWeight: 700 }}>食材在庫管理</div>
        <div style={{ color: "#999", fontSize: 14 }}>ログインしてください</div>
        <button onClick={login} style={{ background: "#2d2926", color: "#fff", border: "none", borderRadius: 24, padding: "12px 28px", fontWeight: 700, fontSize: 14, cursor: "pointer" }}>
          Googleでログイン
        </button>
      </div>
    );
      ...
      <div style={{ background: "#2d2926", padding: "20px 24px 16px", position: "sticky", top: 0, zIndex: 10 }}>
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", maxWidth: 700, margin: "0 auto" }}>
          ...
          <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
            <button onClick={logout} style={{ background: "rgba(255,255,255,0.1)", color: "#c8b99a", border: "none", borderRadius: 16, padding: "6px 14px", fontSize: 12, cursor: "pointer" }}>
              ログアウト
            </button>
            <button onClick={openAdd} style={{ background: "#e8a838", color: "#fff", border: "none", borderRadius: 24, padding: "10px 20px", fontWeight: 700, fontSize: 14, cursor: "pointer" }}>
              ＋ 追加
            </button>
          </div>
        </div>
      ...
      <div style={{ maxWidth: 700, margin: "0 auto", padding: "16px 16px 80px" }}>
        ...
        <div style={{ display: "flex", gap: 6, overflowX: "auto", paddingBottom: 4, marginBottom: 16 }}>
          {["すべて", ...CATEGORIES].map(cat => (
            <button key={cat} onClick={() => setActiveTab(cat)} style={{ background: activeTab === cat ? "#2d2926" : "#fff", color: activeTab === cat ? "#f5f0e8" : "#666", border: "1.5px solid " + (activeTab === cat ? "#2d2926" : "#ddd"), borderRadius: 20, padding: "6px 14px", fontSize: 12, fontWeight: activeTab === cat ? 700 : 400, cursor: "pointer", whiteSpace: "nowrap", flexShrink: 0 }}>
              {cat}
            </button>
          ))}
        </div>
